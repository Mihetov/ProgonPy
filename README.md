...
### Динамическая замена виджетов после сборки EXE

Чтобы менять виджеты **без пересборки**:

1. Собирайте в режиме папки (`dist/ProgonPy/`, не onefile).
2. Используйте внешний каталог рядом с EXE:
   - `dist/ProgonPy/gui/widgets/`
3. Кладите туда/удаляйте оттуда `.py`-файлы виджетов с контрактом:
   - `IS_APP_WIDGET = True`
   - `PANEL_TITLE = "..."`
   - `__init__(parent, client, poller=None, on_log=None)`

При следующем запуске EXE приложение сначала сканирует внешний `gui/widgets` рядом с исполняемым файлом,
поэтому новые/удаленные виджеты автоматически появятся/исчезнут в левом меню.
